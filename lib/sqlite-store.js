...
const { ENGINE_VERSION, selectDiverseFeed } = require("./discovery-engine");
const {
...
const { ageHoursFromPublishedAt, decodeHtmlEntities } = require("./youtube");
...
function createSqliteStore({ databasePath, migrationsDir }) {
  ...
  function getPlayer(playerId) {
    const row = database.prepare(`
      SELECT id, balance, field_value, created_at, updated_at
      FROM players
      WHERE id = ?
    `).get(playerId);
    return toPlayer(row);
  ...
  function getPlayerBySession(tokenHash, now = Date.now()) {
    const row = database.prepare(`
      SELECT p.id, p.balance, p.field_value, p.created_at, p.updated_at
  ...
  function calculatePlayerFieldValue(playerId) {
    const player = database.prepare("SELECT balance FROM players WHERE id = ?").get(playerId);
    if (!player) return null;
    const positions = database.prepare(`
      SELECT p.entry_views, p.early_bonus, v.latest_views
      FROM positions p
      JOIN videos v ON v.id = p.video_id
      WHERE p.player_id = ? AND p.status = 'active'
    `).all(playerId);
    return Number(player.balance) + positions.reduce((total, position) => (
      total + calculatePayout(
        position.entry_views,
        Math.max(position.entry_views, Number(position.latest_views ?? position.entry_views)),
        position.early_bonus,
      )
    ), 0);
  }

  function refreshPlayerFieldValue(playerId, updatedAt = isoNow()) {
    const fieldValue = calculatePlayerFieldValue(playerId);
    if (fieldValue === null) return null;
    database.prepare(`
      UPDATE players
      SET field_value = ?, updated_at = ?
      WHERE id = ? AND field_value <> ?
    `).run(fieldValue, updatedAt, playerId, fieldValue);
    return fieldValue;
  }

  function refreshPlayersForVideo(videoId, updatedAt = isoNow()) {
    database.prepare(`
      SELECT DISTINCT player_id
      FROM positions
      WHERE video_id = ? AND status = 'active'
    `).all(videoId).forEach((row) => refreshPlayerFieldValue(row.player_id, updatedAt));
  }

  function refreshAllPlayerFieldValues(updatedAt = isoNow()) {
    database.prepare("SELECT id FROM players").all()
      .forEach((row) => refreshPlayerFieldValue(row.id, updatedAt));
  }

  function repairEncodedVideoTitles() {
    const update = database.prepare("UPDATE videos SET title = ? WHERE id = ?");
    database.prepare("SELECT id, title FROM videos WHERE title LIKE '%&%;%'").all()
      .forEach((row) => {
        const decoded = decodeHtmlEntities(row.title);
        if (decoded !== row.title) update.run(decoded, row.id);
      });
  }
  ...
  function addSnapshotByVideoId(videoId, snapshot) {
    ...
    const updated = database.prepare(`
      ...
      SET latest_views = ?,
          ...
          updated_at = ?
      WHERE id = ?
        AND (latest_snapshot_at IS NULL OR latest_snapshot_at <= ?)
    `).run(
      Math.max(0, Math.round(Number(snapshot.views))),
      hasFiniteNumber(snapshot.likes) ? Math.max(0, Math.round(Number(snapshot.likes))) : null,
      hasFiniteNumber(snapshot.comments) ? Math.max(0, Math.round(Number(snapshot.comments))) : null,
    ...
    if (updated > 0) refreshPlayersForVideo(videoId, capturedAt);
    return true;
  }
  ...
  function upsertVideo(video, { lane = "direct", candidate = true } = {}, now = Date.now()) {
    ...
    `).run(
      ...
      video.youtubeId,
      video.url,
      decodeHtmlEntities(video.title || "제목 미확인"),
    ...
    const stored = database.prepare("SELECT id FROM videos WHERE youtube_id = ?")
      .get(video.youtubeId);
    if (video.categoryId) {
      database.prepare("UPDATE videos SET category_id = ? WHERE id = ?")
        .run(video.categoryId, stored.id);
    }
  ...
  function addSnapshotByYoutubeId(youtubeId, snapshot) {
    ...
    if (snapshot.publishedAt || snapshot.categoryId || hasFiniteNumber(snapshot.durationSeconds)) {
      database.prepare(`
        UPDATE videos
  ...
  function watchRows(playerId) {
    return database.prepare(`
      SELECT
        ...
        v.*, fc.signal_score, fc.signal_label, fc.signal_reason,
        fc.views_per_hour, fc.growth_rate_per_hour, fc.acceleration,
        fc.engagement_rate, fc.confidence, fc.score_delta,
        fc.snapshot_count, fc.engine_version,
  ...
  function getBootstrap(playerId, now = Date.now()) {
    ...
    return {
      ...
      watches: watchRows(playerId).map((row) => ({
        ...
        growthRatePerHour: row.growth_rate_per_hour,
  ...
  function listFeed(playerId, { sort = "signal", limit = 30 } = {}, now = Date.now()) {
    const normalizedSort = ["signal", "rising", "new", "early"].includes(sort)
      ? sort
      : "signal";
    ...
    const rows = database.prepare(`
      SELECT
        ...
        fc.signal_label, fc.signal_reason, fc.first_seen_at, fc.last_evaluated_at,
        fc.views_per_hour, fc.growth_rate_per_hour, fc.acceleration,
        fc.engagement_rate, fc.confidence, fc.opportunity_score,
        fc.score_delta, fc.snapshot_count, fc.engine_version,
        (SELECT COUNT(*) FROM discoveries d WHERE d.video_id = v.id) AS discoverer_count,
        EXISTS(
      ...
      WHERE fc.expires_at > ?
        ...
        AND v.latest_views IS NOT NULL
        AND NOT EXISTS(
          SELECT 1 FROM discoveries mine
          WHERE mine.video_id = v.id AND mine.player_id = ?
        )
      ORDER BY COALESCE(fc.signal_score, -1) DESC, fc.first_seen_at DESC
      LIMIT 500
    `).all(playerId, playerId, playerId, nowIso, playerId);
    const selected = selectDiverseFeed(rows, {
      sort: normalizedSort,
    ...
    const items = selected.map((row) => ({
      ...
      snapshots: snapshotRows(row.id, 12),
    }));
    const summary = database.prepare(`
      SELECT
        COUNT(*) AS total,
        SUM(fc.status = 'observing') AS observing,
        SUM(fc.status = 'breakout') AS breakout,
        SUM(fc.status = 'rising') AS rising,
        MAX(fc.last_evaluated_at) AS last_evaluated_at
      FROM feed_candidates fc
      JOIN videos v ON v.id = fc.video_id
      WHERE fc.expires_at > ?
        AND (v.duration_seconds IS NULL OR v.duration_seconds <= 180)
    ...
    return {
      ...
      meta: {
        ...
        observing: Number(summary.observing) || 0,
        breakout: Number(summary.breakout) || 0,
        rising: Number(summary.rising) || 0,
        returned: items.length,
        lastEvaluatedAt: summary.last_evaluated_at || null,
      },
    };
  }
  ...
  function harvestInsideTransaction(playerId, positionId, now = Date.now()) {
    ...
    refreshPlayerFieldValue(playerId, harvestedAt);
    database.prepare(`
      INSERT INTO wallet_transactions(id, player_id, amount, kind, reference_id, created_at)
      VALUES (?, ?, ?, 'harvest', ?, ?)
  ...
  function plantPosition(
    ...
    return transaction(() => {
      ...
      refreshPlayerFieldValue(playerId, plantedAt);
      database.prepare(`
        INSERT INTO wallet_transactions(id, player_id, amount, kind, reference_id, created_at)
        VALUES (?, ?, ?, 'plant', ?, ?)
  ...
  function candidateMeasurements(now = Date.now()) {
    const rows = database.prepare(`
      SELECT
        v.id, v.youtube_id, v.published_at, v.latest_views,
        fc.lane, fc.signal_score, fc.engine_version
      FROM feed_candidates fc
      JOIN videos v ON v.id = fc.video_id
      WHERE fc.expires_at > ?
    `).all(isoNow(now));
    return rows.map((row) => ({
      ...
      lane: row.lane,
      previousScore: row.signal_score,
      engineVersion: row.engine_version,
      snapshots: snapshotRows(row.id, 12),
    }));
  }

  function updateCandidateSignal(videoId, signal, now = Date.now()) {
    const metrics = signal.metrics || {};
    database.prepare(`
      UPDATE feed_candidates
      SET status = ?, signal_score = ?, signal_label = ?,
          signal_reason = ?, views_per_hour = ?, growth_rate_per_hour = ?,
          acceleration = ?, engagement_rate = ?, confidence = ?,
          opportunity_score = ?, score_delta = ?, snapshot_count = ?,
          engine_version = ?, last_evaluated_at = ?
      WHERE video_id = ?
    `).run(
      signal.status || (Number.isFinite(signal.score) ? "ranked" : "observing"),
      Number.isFinite(signal.score) ? signal.score : null,
      signal.label,
      signal.reason || "데이터 수집 중",
      Number.isFinite(metrics.viewsPerHour) ? metrics.viewsPerHour : null,
      Number.isFinite(metrics.growthRatePerHour) ? metrics.growthRatePerHour : null,
      Number.isFinite(metrics.acceleration) ? metrics.acceleration : null,
      Number.isFinite(metrics.engagementRate) ? metrics.engagementRate : null,
      Number.isFinite(metrics.confidence) ? metrics.confidence : 0,
      Number.isFinite(metrics.opportunityScore) ? metrics.opportunityScore : null,
      Number.isFinite(signal.scoreDelta) ? signal.scoreDelta : 0,
      Number.isFinite(metrics.snapshotCount) ? metrics.snapshotCount : 0,
      signal.engineVersion || ENGINE_VERSION,
      isoNow(now),
      videoId,
    );
  }
  ...
  function systemStatus() {
    ...
    const counts = database.prepare(`
      SELECT
        ...
        (SELECT COUNT(*) FROM feed_candidates
          WHERE expires_at > ? AND status = 'breakout') AS breakout_candidates,
        (SELECT COUNT(*) FROM feed_candidates
          WHERE expires_at > ? AND status = 'rising') AS rising_candidates,
        (SELECT COUNT(*) FROM players) AS players,
        (SELECT COUNT(*) FROM positions WHERE status = 'active') AS active_positions
    `).get(isoNow(), isoNow(), isoNow());
    return {
      ...counts,
      discoveryEngine: ENGINE_VERSION,
  ...
  migrate();
  repairEncodedVideoTitles();
  refreshAllPlayerFieldValues();
  return {
